...
def fighters(request, slug, template='fighters.html'):
    ...
    context = {
        'fighter': fighter,
        'battles1' : Battles.objects.filter(fighters=fighter.id).filter(eventpair__fighters_1=fighter.id),
        'battles2' : Battles.objects.filter(fighters=fighter.id).filter(eventpair__fighters_2=fighter.id),
